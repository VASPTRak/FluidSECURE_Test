            ...
            geocoder.geocode({ 'latLng': latlng }, function (results, status) {
                if (status == google.maps.GeocoderStatus.OK) {
                    if (results[0]) {
                        ...
                        GetTimeZones(lat, lng);
                    }
                }
            });
        }

        function GetTimeZones(latitude, longitude) {
            var weburl = "https://maps.googleapis.com/maps/api/timezone/json?location=" +
                latitude + "," + longitude + "&timestamp=" + (Math.round((new Date().getTime()) / 1000)).toString() + "&sensor=false";
            $.ajax({
                type: "POST",
                url: "FuelSecureUnit.aspx/GetTimeZoneName",
                data: "{Url: '" + weburl + "'}",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (r) {
                    ...
                    $('#<%= DDL_TimeZone.ClientID %>').val(splitdata[0]);
                    if (splitdata.length > 1) {
                        $('#<%= LBL_TimeZone.ClientID %>').text(splitdata[1]);
                    }
                }
            });
    ...
    <div id="datadiv">
        ...
        <asp:UpdatePanel ID="up_Main" runat="server">
            <ContentTemplate>
                                        ...
                                        <ContentTemplate>
                                            <asp:GridView ID="gv_FuelingTimes" CssClass="table table-bordered" runat="server" DataKeyNames="TimeId,TimeText" AutoGenerateColumns="False" EmptyDataText="Data Not found.">
                                                <Columns>
                                                    ...
                                                    <asp:TemplateField HeaderText="Stay Open">
                                                        <ItemTemplate>
                                                            <asp:CheckBox ID="CHK_StayOpen" Enabled="false" runat="server" onclick="javascript:SelectboxStayOpen(this);" />
                                                        </ItemTemplate>
                                                    </asp:TemplateField>
                                                </Columns>
                                            </asp:GridView>
                ...
                <!-- /.modal -->
                <div class="modal fade" tabindex="-1" role="dialog" id="FSLinkHistory">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title text-center">Select form Previous FS Links:</h5>
                            </div>
                            <div class="modal-body">
                                <div class="row col-md-12 col-sm-12">
                                    <asp:Label ID="LBL_LinkHistory" runat="server" Text=""></asp:Label>
                                </div>
                                <div class="row col-md-12 col-sm-12 text-center" style="overflow-x: auto; max-height: 400px;">
                                    <asp:UpdatePanel ID="UP_FSHistory" runat="server">
                                        <ContentTemplate>
                                            <asp:GridView ID="gv_HistoryView" CssClass="table table-bordered" runat="server" DataKeyNames="Name" AutoGenerateColumns="False" 
                                                ...
                                                <Columns>
                                                    <asp:TemplateField HeaderText="">
                                                        <ItemTemplate>
                                                            <asp:RadioButton ID="rbLinks" runat="server" onclick="RadioCheck(this);" />
                                                        </ItemTemplate>
                                                    </asp:TemplateField>
                                                    <asp:BoundField HeaderText="Names" DataField="Name" ReadOnly="True" HeaderStyle-HorizontalAlign="Left" ItemStyle-HorizontalAlign="Left" />
                                                </Columns>
                                            </asp:GridView>
                ...
                <!-- /.modal -->
                <asp:HiddenField ID="HDF_FSCurrentName" runat="server" />
                <asp:HiddenField ID="HDF_FSNameHistory" runat="server" />
                <div class="panel panel-primary" style="margin: 20px;">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="row col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div class="form-group col-md-2 col-sm-2 col-xs-12">
                                <asp:TextBox ID="TXT_OriginalNameOfFluidSecure" runat="server" CssClass="form-control input-sm" TabIndex="26" TextMode="MultiLine" Rows="4"></asp:TextBox>
                        ...
                        <div class="row col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div class="form-group col-md-4 col-sm-4 textright col-xs-12">
                                <label>Reconfigure Link On Next Use:</label>
                            </div>
                            <div class="form-group col-md-2 col-sm-2 col-xs-12">
                                <asp:CheckBox ID="CHK_ReconfigureLink" runat="server" TabIndex="31" />
                            </div>
                        </div>
                        <div class="row col-md-12 col-sm-12 col-xs-12" id="divActivate" runat="server">
                            ...
                            <div class="form-group col-md-2 col-sm-2 col-xs-12">
                                <asp:CheckBox ID="CHK_Activate" runat="server" TabIndex="18" data-toggle="tooltip" title="(If link is repaired user can re-activate the link by clicking the “Activate” checkbox.) " />
                            </div>
                        </div>
    ...
    <script language="javascript">
        ...
        function SelectboxFuelingTimes(spanChk) {
            ...
            if (spanChk.id.toLowerCase().indexOf("chk_fuelingtimes_0") > -1) {
                ...
                var items = $("input[id*='CHK_FuelingTimes']");//Parent.getElementsByTagName('input[id*="CHK_FuelingTimes"');
                for (i = 0; i < items.length; i++) {
                    if (items[i].id != spanChk.id && items[i].type == "checkbox") {
                        if (items[i].checked != IsChecked) {
                            items[i].checked = IsChecked;
                ...
                var itemStayOpen = $("input[id*='CHK_StayOpen']");//Parent.getElementsByTagName('input[id*="CHK_FuelingTimes"');
                for (i = 0; i < itemStayOpen.length; i++) {
                    if (itemStayOpen[i].id != spanChk.id && itemStayOpen[i].type == "checkbox") {
                        var stayOpenElement = $("#" + itemStayOpen[i].id + "");
                        if (IsChecked) {
                            stayOpenElement.removeAttr("disabled");
                        }
                        else {
                            stayOpenElement.attr("disabled", "disabled");
                        }
                    }
                }
                ...
                var elementPosition = spanChk.id.replace("MainContent_gv_FuelingTimes_CHK_FuelingTimes_", "");
                var stayOpenElement = $("#MainContent_gv_FuelingTimes_CHK_StayOpen_" + elementPosition + "");
                ...
                if (IsChecked == false) {
                    ...
                    Parent = document.getElementById('<%= gv_FuelingTimes.ClientID %>');
                    var items = $("input[id*='CHK_FuelingTimes']");//Parent.getElementsByTagName('input');
                    for (i = 0; i < items.length; i++) {
        ...
        function SelectboxStayOpen(spanChk) {
            debugger
            if (spanChk.id.toLowerCase().indexOf(("chk_stayopen_0").toLowerCase()) > -1) {
                var IsChecked = spanChk.checked;

                Parent = document.getElementById('<%= gv_FuelingTimes.ClientID %>');
                var items = $("input[id*='CHK_StayOpen']");//Parent.getElementsByTagName('input[id*="CHK_FuelingTimes"');
                for (i = 0; i < items.length; i++) {
                    if (items[i].id != spanChk.id && items[i].type == "checkbox") {
                        if (items[i].checked != IsChecked) {
                            items[i].checked = IsChecked;
                ...
                var checkBoxSelector = "#<%=gv_FuelingTimes.ClientID%> input[id*='CHK_StayOpen_0']";
                ...
                if (IsChecked == false) {
                    ...
                    Parent = document.getElementById('<%= gv_FuelingTimes.ClientID %>');
                    var items = $("input[id*='CHK_StayOpen']");//Parent.getElementsByTagName('input');
                    for (i = 0; i < items.length; i++) {
                        if (items[i].type == "checkbox" && !(items[i].id.toLowerCase().indexOf("chk_stayopen_0") > -1)) {
                            if (items[i].checked == true) {
                                isAll = true;
                            }
                            else {
