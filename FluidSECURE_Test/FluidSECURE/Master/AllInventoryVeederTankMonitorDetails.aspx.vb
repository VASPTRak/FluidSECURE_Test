...
Public Class AllInventoryVeederTankMonitorDetails
	Inherits System.Web.UI.Page

	Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(AllInventoryVeederTankMonitorDetails))
	...
	Private Sub BindColumns()
		Try
			...
			dtColumns = OBJMaster.GetColumnNameForSearchForInventoryVeeder()
	...
	Protected Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
		Try
			...
			ElseIf ((DDL_Customer.SelectedValue <> 0) And DDL_ColumnName.SelectedValue <> "0") Then
				strConditions = IIf(strConditions = "", " and TMD." + DDL_ColumnName.SelectedValue + " = " + DDL_Customer.SelectedValue + "", strConditions + " and TMD." + DDL_ColumnName.SelectedValue + " = " + DDL_Customer.SelectedValue + "")
			End If
			...
			OBJMaster = New MasterBAL()
			Dim dtTankMonitor As DataTable = New DataTable()


			dtTankMonitor = OBJMaster.GetInventoryVeederTankMonitorDetailsbyCondition(strConditions, Convert.ToInt32(Session("PersonId").ToString()), Session("RoleId").ToString())

			Session("dtTankMonitor") = dtTankMonitor

			gvInventoryVeederTankMonitor.DataSource = dtTankMonitor
			gvInventoryVeederTankMonitor.DataBind()

			ViewState("Column_Name") = "VR_InventoryId"
			ViewState("Sort_Order") = "desc"
			RebindData("VR_InventoryId", "desc")
	...
	Private Sub RebindData(sColimnName As String, sSortOrder As String)
		Try
			Dim dt As DataTable = CType(Session("dtTankMonitor"), DataTable)
			dt.DefaultView.Sort = sColimnName + " " + sSortOrder
			gvInventoryVeederTankMonitor.DataSource = dt
			gvInventoryVeederTankMonitor.DataBind()
	...
	Protected Sub gvInventoryVeederTankMonitor_Sorting(sender As Object, e As GridViewSortEventArgs) Handles gvInventoryVeederTankMonitor.Sorting
		Try
			...
			End If
		Catch ex As Exception
			log.Error("Error occurred in gvInventoryVeederTankMonitor_Sorting Exception is :" + ex.Message)
			ErrorMessage.Visible = True
			ErrorMessage.InnerText = "Error occurred while getting data, please try again later."
		End Try
	End Sub

	Protected Sub gvInventoryVeederTankMonitor_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles gvInventoryVeederTankMonitor.PageIndexChanging
		Try
			gvInventoryVeederTankMonitor.PageIndex = e.NewPageIndex
			Dim dtTankMonitor As DataTable = Session("dtTankMonitor")

			gvInventoryVeederTankMonitor.DataSource = dtTankMonitor
			gvInventoryVeederTankMonitor.DataBind()

		Catch ex As Exception
			log.Error("Error occurred in gvInventoryVeederTankMonitor_PageIndexChanging Exception is :" + ex.Message)
		End Try
	...
	'End Function	'<System.Web.Services.WebMethod(True)>
	'Public Shared Function DeleteRecord(ByVal TankId As String) As String
	'	Dim beforeData As String = ""
	'	Dim OBJMaster = New MasterBAL()
	'	If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
