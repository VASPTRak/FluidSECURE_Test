...
Public Class Vehicle
			...
			Else
				If Not IsPostBack Then
					...
					If (Not Request.QueryString("VehicleId") = Nothing And Not Request.QueryString("VehicleId") = "") Then
						...
						ElseIf (Request.QueryString("RecordIs") = "edited") Then
							message.Visible = True
							message.InnerText = "Record saved"
						End If
					...
					End If
					'RBL_UnitType_SelectedIndexChanged(Nothing, Nothing)
			...
			If dtCustInfo IsNot Nothing Then
				If dtCustInfo.Rows.Count > 0 Then
					If dtCustInfo.Rows(0)("FuelingType") = "False" Then
						lblFuelType.Text = "Total Miles allowed between Fueling"
						txtOdoLimit.ToolTip = """Total Miles allowed between Fueling"” represents the maximum number of Miles the vehicle is allowed to travel between fueling. Example if a vehicle’s current odometer is 1000 and the Total   between Fueling is set to 300, the only odometer that will be accepted is between 1000 to 1300. Note: if choosing this option, the Check Odometer/Hours Reasonability must be checked."
						RBL_UnitType.SelectedValue = "1"
					ElseIf dtCustInfo.Rows(0)("FuelingType") = "True" Then
						lblFuelType.Text = "Total Kilometers allowed between Fueling"
						txtOdoLimit.ToolTip = """Total Kilometers allowed between Fueling"” represents the maximum number of Kilometers the vehicle is allowed to travel between fueling. Example if a vehicle’s current odometer is 1000 and the Total   between Fueling is set to 300, the only odometer that will be accepted is between 1000 to 1300. Note: if choosing this option, the Check Odometer/Hours Reasonability must be checked."
						RBL_UnitType.SelectedValue = "2"
					Else
						lblFuelType.Text = "Total Miles/Kilometers allowed between Fueling"
						txtOdoLimit.ToolTip = """Total Miles/Kilometers allowed between Fueling"” represents the maximum number of Miles/Kilometers the vehicle is allowed to travel between fueling. Example if a vehicle’s current odometer is 1000 and the Total   between Fueling is set to 300, the only odometer that will be accepted is between 1000 to 1300. Note: if choosing this option, the Check Odometer/Hours Reasonability must be checked."
						RBL_UnitType.SelectedValue = "1"
					End If
				End If
			End If
	...
	Private Function CreateAfterData(VehicleId As Integer, IsBefore As Boolean) As String
		Try
			...
			Dim data As String = "VehicleId = " & VehicleId & " ; " &
									...
									"Current Firmware Version = " & txt_FirmwareVer.Text.Replace(",", " ") & " ; " &
	...
	Private Sub ValidateAndSaveVehicle(IsSaveAndAddNew As Boolean)
		...
		Try
			...
			OBJMaster = New MasterBAL()
			Dim FSTagMacAddress As String = ""
			If txtFSTagMacAddress.Text <> "" Then
				'check and remove colon and lower case
				FSTagMacAddress = txtFSTagMacAddress.Text.Replace(":", "").Replace(" ", "").ToLower()
				CheckVehicleFSTagMacAddressExist = OBJMaster.CheckVehicleFSTagMacAddressExist(FSTagMacAddress, vehicleId, Convert.ToInt32(DDL_Customer.SelectedValue))
			End If
			...
			If (CHK_CheckOdometerReasonable.Checked = True And txtOdoLimit.Text = "" And CHK_RequireOdometerEntry.Checked = True) Then
				...
				ErrorMessage.Visible = True
				If RBL_UnitType.SelectedValue = "1" Then
					ErrorMessage.InnerText = "Please enter total miles allowed between fueling."
				Else
					ErrorMessage.InnerText = "Please enter total kilometers allowed between fueling."
				End If
			...
			If (CHK_CheckOdometerReasonable.Checked = True And txtOdoLimit.Text = "0" And CHK_RequireOdometerEntry.Checked = True) Then
				...
				Else
					ErrorMessage.InnerText = "Please enter total kilometers allowed between fueling greater than 0."
				End If
			...
			result = OBJMaster.SaveUpdateVehicle(vehicleId, txtVehicleName.Text, txtMake.Text, txtModel.Text,
												 ...
												 TXT_FoBNUM.Text.Replace(" ", ""), CHK_Active.Checked, FSTagMacAddress, IIf(txtCurrentHrs.Text = "", "-1", txtCurrentHrs.Text), IIf(txtHoursLimit.Text = "", "-1", txtHoursLimit.Text),
			...
			If result > 0 Then
				If (vehicleId > 0) Then
					...
					Else
						Response.Redirect(String.Format("~/Master/Vehicle?VehicleId={0}&RecordIs=edited", result))
					End If
				Else
					If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
	...
	'Protected Sub RBL_UnitType_SelectedIndexChanged(sender As Object, e As EventArgs)
	'	If RBL_UnitType.SelectedValue = "1" Then
	'		lblFuelType.Text = "Total Miles allowed between Fueling"
	'		txtOdoLimit.ToolTip = """Total Miles allowed between Fueling"” represents the maximum number of Miles the vehicle is allowed to travel between fueling. Example if a vehicle’s current odometer is 1000 and the Total   between Fueling is set to 300, the only odometer that will be accepted is between 1000 to 1300. Note: if choosing this option, the Check Odometer/Hours Reasonability must be checked."
	'	ElseIf RBL_UnitType.SelectedValue = "2" Then
	'		lblFuelType.Text = "Total Kilometers allowed between Fueling"
	'		txtOdoLimit.ToolTip = """Total Kilometers allowed between Fueling"” represents the maximum number of Kilometers the vehicle is allowed to travel between fueling. Example if a vehicle’s current odometer is 1000 and the Total   between Fueling is set to 300, the only odometer that will be accepted is between 1000 to 1300. Note: if choosing this option, the Check Odometer/Hours Reasonability must be checked."
	'	Else
	'		lblFuelType.Text = "Total Miles/Kilometers allowed between Fueling"
