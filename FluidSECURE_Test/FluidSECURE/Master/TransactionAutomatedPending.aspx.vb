...
Public Class TransactionAutomatedPending
	Inherits System.Web.UI.Page

	Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(TransactionAutomatedPending))
	...
	Protected Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
		Try
			...
			If (DDL_Missed.SelectedValue = "-1") Then
				strConditions = IIf(strConditions = "", " and ISNULL(T.TransactionStatus,0) <> 2 and datediff(minute, T.[CreatedDate] ,getdate()) > 15 ", strConditions + " and ISNULL(T.TransactionStatus,0) <> 2 and datediff(minute, T.[CreatedDate] ,getdate()) > 15 ")
			Else
		...
		Try
			...
			If (e.Row.RowType = DataControlRowType.DataRow) Then
				...
				Try
					...
					'		If (TransactionStatus <> ConfigurationManager.AppSettings("CompletedText").ToString()) And (TransactionStatus <> ConfigurationManager.AppSettings("OngoingText").ToString()) Then
					'			e.Row.Visible = True
					'		Else
					'			e.Row.Visible = False
					'		End If
	...
	Private Sub BindTransactionStatus()
		Try
			...
			DDL_Missed.Items.Insert(1, New ListItem(ConfigurationManager.AppSettings("NotStartedText").ToString(), "UserStopped"))
			DDL_Missed.Items.Insert(2, New ListItem(ConfigurationManager.AppSettings("MissedText").ToString(), "IsMissed"))
	...
	Private Function GetTransactionsPageWise(ByVal pageIndex As Integer, strConditions As String, CustomSortFields As String, CustomSortDirection As String,
		...
		If (firstLoad = True Or CustomSortFields = "TransactionId") Then
			dsT = OBJMaster.GetFATransactionsByCondition(strConditions, Session("PersonId").ToString(), Session("RoleId").ToString(), False,
													   pageIndex, 20, 1, "", "")
