...
Public Class AllDeliveryVeederTankMonitorDetails
	Inherits System.Web.UI.Page


	Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(AllDeliveryVeederTankMonitorDetails))
	...
	Private Sub BindColumns()
		Try
			...
			dtColumns = OBJMaster.GetColumnNameForSearchForDeliveryVeeder()
	...
	Protected Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
		Try
			...
			dtTankMonitor = OBJMaster.GetDeliveryVeederTankMonitorDetailsbyCondition(strConditions, Convert.ToInt32(Session("PersonId").ToString()), Session("RoleId").ToString())

			Session("dtTankMonitor") = dtTankMonitor

			gvDeliveryVeederTankMonitor.DataSource = dtTankMonitor
			gvDeliveryVeederTankMonitor.DataBind()

			ViewState("Column_Name") = "VR_DeliveryId"
			ViewState("Sort_Order") = "desc"
			RebindData("VR_DeliveryId", "desc")
	...
	Private Sub RebindData(sColimnName As String, sSortOrder As String)
		Try
			...
			gvDeliveryVeederTankMonitor.DataSource = dt
			gvDeliveryVeederTankMonitor.DataBind()
	...
	Protected Sub gvInventoryVeederTankMonitor_Sorting(sender As Object, e As GridViewSortEventArgs) Handles gvDeliveryVeederTankMonitor.Sorting
		Try
	...
	Protected Sub gvDeliveryVeederTankMonitor_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles gvDeliveryVeederTankMonitor.PageIndexChanging
		Try
			gvDeliveryVeederTankMonitor.PageIndex = e.NewPageIndex
		...
		Catch ex As Exception
			log.Error("Error occurred in gvDeliveryVeederTankMonitor_PageIndexChanging Exception is :" + ex.Message)
		End Try
