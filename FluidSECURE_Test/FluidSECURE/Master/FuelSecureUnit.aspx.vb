...
Public Class FuelSecureUnit
            ...
            Else
                ...
                Else
                    ...
                    If chk_EnableDisableLinkName.Checked Then
                        txtwifissid.Enabled = True
                        gv_HistoryView.Columns(0).Visible = True
                    Else
                        txtwifissid.Enabled = False
                        gv_HistoryView.Columns(0).Visible = False
                    End If
    ...
    Private Sub BindHoseDetails(SiteId As Integer)
        Try
            ...
            If (dtHose.Rows.Count > 0) Then
                ...
                HDF_FSCurrentName.Value = dtHose.Rows(0)("WifiSSId").ToString()
                HDF_FSNameHistory.Value = dtHose.Rows(0)("HistoryNameOfFluidSecure").ToString()
                ...
                Count = OBJMaster.CheckSiteIdPersonIdGateHubCount(SiteId)

                If (Count = 1) Then
                    gv_FuelingTimes.Columns(2).Visible = True
                Else
                    gv_FuelingTimes.Columns(2).Visible = False
                End If
            ...
            If dtTiming IsNot Nothing Then
                For Each dr As DataRow In dtTiming.Rows
                    ...
                    For Each rows As GridViewRow In gv_FuelingTimes.Rows
                        If (dr("TimeId") = gv_FuelingTimes.DataKeys(rows.RowIndex).Values("TimeId").ToString()) Then
                            ...
                            If (dr("StayOpenGate").ToString() = "True") Then
                                TryCast(rows.FindControl("CHK_StayOpen"), CheckBox).Checked = True
                            End If

                            TryCast(rows.FindControl("CHK_StayOpen"), CheckBox).Enabled = True
                    ...
                    If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
                        ...
                        beforeStayOpen = IIf(beforeFuelingTimes = "", dr("TimeText") & ":" & dr("StayOpenGate").ToString(), beforeFuelingTimes & ";" & dr("TimeText") & ":" & dr("StayOpenGate").ToString())
                    End If
                Next
    ...
    Private Function CreateData(SiteID As Integer, IsBefore As Boolean) As String
        Try
            ...
            Dim data As String = "SiteID = " & SiteID & " ; " &
                                    ...
                                    "Stay Gate Open = " & FuelingStayOpen & " ; " &
    ...
    Private Sub ValidateAndSaveFSLink(IsSaveAndAddNew As Boolean, IsCheckTransfer As Boolean)
        ...
        Try
            ...
            Dim OriginalNameOfFluidSecure As String = ""
            Dim FSLinkHistory As String = ""
            If txtwifissid.Text = HDF_FSCurrentName.Value Then
                OriginalNameOfFluidSecure = TXT_OriginalNameOfFluidSecure.Text
                FSLinkHistory = HDF_FSNameHistory.Value
            ElseIf HDF_FSCurrentName.Value = "" Then
                If txtwifissid.Text <> TXT_OriginalNameOfFluidSecure.Text And txtwifissid.Text.StartsWith("FS-") Then
                    OriginalNameOfFluidSecure = TXT_OriginalNameOfFluidSecure.Text & "," & txtwifissid.Text
                    'FSLinkHistory = TXT_OriginalNameOfFluidSecure.Text
                Else
                    OriginalNameOfFluidSecure = TXT_OriginalNameOfFluidSecure.Text
                    'FSLinkHistory = TXT_OriginalNameOfFluidSecure.Text
                End If
            Else
                If HDF_FSCurrentName.Value.StartsWith("FS-") Then
                    If txtwifissid.Text <> TXT_OriginalNameOfFluidSecure.Text And txtwifissid.Text.StartsWith("FS-") Then
                        OriginalNameOfFluidSecure = TXT_OriginalNameOfFluidSecure.Text & "," & txtwifissid.Text
                        FSLinkHistory = HDF_FSNameHistory.Value + "," + HDF_FSCurrentName.Value
                    Else
                        OriginalNameOfFluidSecure = TXT_OriginalNameOfFluidSecure.Text
                        FSLinkHistory = HDF_FSNameHistory.Value + "," + HDF_FSCurrentName.Value
                    End If
            ...
            If SiteId = 0 Then
                ...
                result = OBJMaster.SaveUpdateSite(SiteId, 0, Nothing, txtSiteAddress.Text, txt_ExportCode.Text, ddlCustomer.SelectedValue, Convert.ToInt32(Session("PersonId")), Lat, Lng,
                                                 ...
                                                 "N", Nothing, txt_ReplaceableHoseName.Text, txtUnitsmeasured.Text, txt_Pulses.Text, DDL_TimeZone.SelectedValue, OriginalNameOfFluidSecure,
                                                 CHK_DisableGeoLocation.Checked, CHK_IsBusy.Checked, txtIpAddress.Text.ToLower(), txt_FirmwareVer.Text, Convert.ToInt32(txtSwitchTimeB.Text), dttankInfo.Rows(0)("TankId").ToString(), DisplayOrder,
                                                  NumberOfZeroTransaction, CHK_Activate.Checked, txtFSNPMacAddress.Text, FSLinkHistory, CHK_ReconfigureLink.Checked) ', chk_isReplaced.Checked
                'Else
                '    result = OBJMaster.SaveUpdateSite(SiteId, 0, Nothing, txtSiteAddress.Text, txt_ExportCode.Text, ddlCustomer.SelectedValue, Convert.ToInt32(Session("PersonId")), Lat, Lng,
            ...
            If result > 0 Then
                Try
                    ...
                    dtTimings.Columns.Add("StayOpenGate", System.Type.[GetType]("System.Boolean"))
                    ...
                    For Each item As GridViewRow In gv_FuelingTimes.Rows
                        ...
                        Dim CHK_StayOpen As CheckBox = TryCast(item.FindControl("CHK_StayOpen"), CheckBox)
                        If (CHK_FuelingTimes.Checked = True) Then
                            ...
                            dr("StayOpenGate") = CHK_StayOpen.Checked
                            ...
                            If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
                                ...
                                afterStayOpen = IIf(afterStayOpen = "", gv_FuelingTimes.DataKeys(item.RowIndex).Values("TimeText").ToString() & ":" & "StayOpenGate - " & IIf(CHK_StayOpen.Checked, "True", "False"), afterStayOpen & ";" & gv_FuelingTimes.DataKeys(item.RowIndex).Values("TimeText").ToString() & ":" & "StayOpenGate - " & IIf(CHK_StayOpen.Checked, "True", "False"))
                            End If
    ...
    Protected Sub btn_ViewHistory_Click(sender As Object, e As EventArgs)
        ...
        End Try
    End Sub
    Private Sub BindHistory(SiteId)
        Try

            Dim dtFSHistory As DataTable = New DataTable()
            OBJMaster = New MasterBAL()
            dtFSHistory = OBJMaster.GetHistory(SiteId)
        ...
        Catch ex As Exception
            ...
            log.Error("Error occurred in BindFSnames Exception is :" + ex.Message)
            ErrorMessage.Visible = True
            ErrorMessage.InnerText = "Error occurred while getting FluidSecure Link Names , please try again later."
    ...
    Protected Sub btnFSHistoryOk_Click(sender As Object, e As EventArgs)
        For i As Integer = 0 To gv_HistoryView.Rows.Count - 1
            ...
            Dim rb As RadioButton = DirectCast(gv_HistoryView.Rows(i).Cells(0).FindControl("rbLinks"), RadioButton)
            If rb IsNot Nothing Then
                If rb.Checked = True Then

                    Dim Name = gv_HistoryView.DataKeys(i).Value.ToString()
                    txtwifissid.Text = Name
                End If
