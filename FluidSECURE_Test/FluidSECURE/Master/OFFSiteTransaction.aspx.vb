...
Public Class OFFSiteTransaction
	Inherits System.Web.UI.Page

	Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(Transaction))
			...
			Else
				If Not IsPostBack Then
					...
					If (Not Request.QueryString("TransactionId") = Nothing And Not Request.QueryString("TransactionId") = "") Then
						...
						lblHeader.Text = "Edit Transaction Information"
		...
		Try
			...
			If (dtTransaction.Rows.Count > 0) Then
				...
				If TransactionStatus <> ConfigurationManager.AppSettings("CompletedText").ToString() And TransactionStatus <> ConfigurationManager.AppSettings("MissedText").ToString() Then
					...
					Response.Redirect("~/Master/AllOFFSiteTransactions.aspx?Filter=Filter")
				...
				HDF_TotalTransactions.Value = OBJMaster.GetTransactionIdByCondition(TransactionId, False, False, False, False, True, Convert.ToInt32(Session("PersonId").ToString()), Session("RoleId").ToString(), Session("CustomerId").ToString(), 1, False, strConditions)
				...
				txtCost.Text = dtTransaction.Rows(0)("TransactionCost")
		...
		Try
			...
			If result > 0 Then
				...
				If txtCost.Text <> "" Then
					Dim check As String = txtCost.Text
					OBJMaster.UpdateTransactionCost(result, txtCost.Text)
				End If
				...
				Else
					...
					Response.Redirect(String.Format("~/Master/OFFSiteTransaction?TransactionId={0}&RecordIs=New", result))
				End If
			...
			Else
				If (TransactionId > 0) Then
					...
					ErrorMessage.Visible = True
					ErrorMessage.InnerText = "Transction update failed, please try again"
				Else
					If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
						Dim writtenData As String = CreateData(result)
	...
	Protected Sub First_Click(sender As Object, e As EventArgs) Handles btnFirst.Click
		Try
			...
			End If
			OBJMaster = New MasterBAL()
			Dim TransactionId As Integer = OBJMaster.GetTransactionIdByCondition(CurrentTransactionId, True, False, False, False, False, Convert.ToInt32(Session("PersonId").ToString()), Session("RoleId").ToString(), Session("CustomerId").ToString(), 1, False, strConditions)
	...
	Protected Sub btnprevious_Click(sender As Object, e As EventArgs) Handles btnprevious.Click
		Try
			...
			End If
			OBJMaster = New MasterBAL()
			Dim TransactionId As Integer = OBJMaster.GetTransactionIdByCondition(CurrentTransactionId, False, False, False, True, False, Convert.ToInt32(Session("PersonId").ToString()), Session("RoleId").ToString(), Session("CustomerId").ToString(), 1, False, strConditions)
			hdfTransactionId.Value = TransactionId
			BindTransactionsDetails(TransactionId)
		Catch ex As Exception
	...
	Protected Sub btnNext_Click(sender As Object, e As EventArgs) Handles btnNext.Click
		Try
			...
			End If
			OBJMaster = New MasterBAL()
			Dim TransactionId As Integer = OBJMaster.GetTransactionIdByCondition(CurrentTransactionId, False, False, True, False, False, Convert.ToInt32(Session("PersonId").ToString()), Session("RoleId").ToString(), Session("CustomerId").ToString(), 1, False, strConditions)
			hdfTransactionId.Value = TransactionId
			BindTransactionsDetails(TransactionId)
		Catch ex As Exception
	...
	Protected Sub btnLast_Click(sender As Object, e As EventArgs) Handles btnLast.Click
		Try
			...
			End If
			OBJMaster = New MasterBAL()
			Dim TransactionId As Integer = OBJMaster.GetTransactionIdByCondition(CurrentTransactionId, False, True, False, False, False, Convert.ToInt32(Session("PersonId").ToString()), Session("RoleId").ToString(), Session("CustomerId").ToString(), 1, False, strConditions)
			hdfTransactionId.Value = TransactionId
			BindTransactionsDetails(TransactionId)
		Catch ex As Exception
    ...
    Private Function CreateData(TransactionId As Integer) As String
		Try
            ...
            Dim data As String = "TransactionId = " & TransactionId & " ; " &
                                    ...
                                    "Cost = " & txtCost.Text.Trim().Replace(",", " ") & " ; " &
