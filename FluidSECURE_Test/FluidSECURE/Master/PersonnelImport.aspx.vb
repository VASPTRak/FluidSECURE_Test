...
Public Class PersonnelImport
	Inherits System.Web.UI.Page

	Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(PersonnelImport))
	...
	Protected Sub btnUpload_Click(sender As Object, e As EventArgs)
		Try
			...
			If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
				CSCommonHelper.WriteLog("Import", "Personnel", "File Name = " & FU_Person.FileName.Replace(",", " ") & " ; Company =  " & ddlCustomer.SelectedItem.Text.Replace(",", " ") &
	...
	Protected Function ConvertStringIntoDatTableAndInsertData(data As String) As String
		...
		Dim pattern As String = "^[- +()]*[0-9][- +()0-9]*$" '"^(?:(?:\+?1\s*(?:[.-]\s*)?)?(?:\(\s*([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9])\s*\)|([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9]))\s*(?:[.-]\s*)?)?([2-9]1[02-9]|[2-9][02-9]1|[2-9][02-9]{2})\s*(?:[.-]\s*)?([0-9]{4})(?:\s*(?:#|x\.?|ext\.?|extension)\s*(\d+))?$"
		...
		Try
			...
			For Each dr As DataRow In dt.Rows
				...
				If (dr("PinNumber") <> "") Then
					'Dim number As Long
					'If (Int64.TryParse(dr("PinNumber").ToString(), number)) Then
				...
				If (dr("PhoneNumber") <> "") Then
					...
					If (Regex.Match(dr("PhoneNumber"), pattern).Success) Then
						...
						Dim CheckNUmberExists = OBJMaster.PhoneNumberIsExists(dr("PhoneNumber"), 0)

						If CheckNUmberExists = True Then
							strLog = strLog & Environment.NewLine & currentDateTime & "--" & dr("PhoneNumber") & " is already exist. Check Row  " & rowIndex & " & column 7 in uploaded file."
							isDirty = True
						End If
					Else
						strLog = strLog & Environment.NewLine & currentDateTime & "-- " & dr("PhoneNumber") & " is in wrong format. Please enter valid contact number. Only (,),-, space, and + symbols allowed. Check Row  " & rowIndex & " & column 7 in uploaded file."
						isDirty = True
					End If

				Else
					strLog = strLog & Environment.NewLine & currentDateTime & "--" & "Phone Number field is required. Check Row  " & rowIndex & " & column 7 in uploaded file."
					isDirty = True
				End If
				...
				If (dr("Email") <> "") Then
					...
					Dim dtDetails As DataTable = New DataTable()
					dtDetails = OBJMaster.GetPersonnelByNameAndNumberAndEmail(" and ANU.Email = '" & dr("Email") & "'", Session("PersonId").ToString(), Session("RoleId").ToString())

					If dtDetails.Rows.Count > 0 Then
						strLog = strLog & Environment.NewLine & currentDateTime & "--" & dr("Email") & " is already exist. Check Row  " & rowIndex & " & column 8 in uploaded file."
						isDirty = True
					Else
						If (dr("Email").ToString().Length > 256) Then
							strLog = strLog & Environment.NewLine & currentDateTime & "--" & "Email (" & dr("Email") & ") is must be less than equal to 256 characters. Check Row  " & rowIndex & " & column 8 in uploaded file."
							isDirty = True
						End If
					End If
				Else
					strLog = strLog & Environment.NewLine & currentDateTime & "--" & "Email field is required. Check Row  " & rowIndex & " & column 8 in uploaded file."
					isDirty = True
				End If
				...
				If (dr("ExportCode").ToString().Length > 25) Then
					strLog = strLog & Environment.NewLine & currentDateTime & "--" & " Export Code (" & dr("ExportCode") & ") is must be less than equal to 25 characters. Check Row  " & rowIndex & " & column 9 in uploaded file."
					isDirty = True
				End If
				...
				Else
					strLog = strLog & Environment.NewLine & currentDateTime & "--" & "Active Flag (" & dr("Active") & ") is must be Y or N. Check Row  " & rowIndex & " & column 10 in uploaded file."
					isDirty = True
				End If
				...
				If (dr("FluidLimitperday").ToString() <> "") Then
					...
					Else
						strLog = strLog & Environment.NewLine & currentDateTime & "--" & "Fluid Limit per day (" & dr("FluidLimitperday") & ") is must be in number format. Check Row  " & rowIndex & " & column 11 in uploaded file."
						isDirty = True
					End If
				Else
					'strLog = strLog & Environment.NewLine & currentDateTime & "--" & "Fluid Limit per day is blank. Fill this value on UI later.Check Row  " & rowIndex & " & column 10 in uploaded file."
				End If
				...
				If (dr("FluidLimitpertransaction").ToString() <> "") Then
					...
					Else
						strLog = strLog & Environment.NewLine & currentDateTime & "--" & "Fluid Limit per transaction (" & dr("FluidLimitpertransaction") & ") is must be in number format. Check Row  " & rowIndex & " & column 12 in uploaded file."
						isDirty = True
					End If
				Else
					'strLog = strLog & Environment.NewLine & currentDateTime & "--" & "Fluid Limit per transaction is blank. Fill this value on UI later.Check Row  " & rowIndex & " & column 11 in uploaded file."
				End If
      ...
      .IsGateHub = False
			...
			If result.Succeeded Then
				...
				Try
					...
					'Insert Vehicle and Fuel mapping timing
	...
	Protected Sub lnkTemplate_Click(sender As Object, e As EventArgs)
		...
		Dim path As String = Server.MapPath("\Content\Templates\PersonnelImportTemplate.csv")
