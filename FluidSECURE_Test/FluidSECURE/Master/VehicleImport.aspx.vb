            ...
            For Each dr As DataRow In dt.Rows
                ...
                If (dr("DepartmentNo") <> "") Then
                    ...
                    If (dr("DepartmentId") = "") Then
                        ...
                        flagCheckDept = 1
                        strErrorDept = currentDateTime & "--" & "Vehicle department number (" & dr("DepartmentNo") & ") is not found. Check Row  " & rowIndex & " & column 4 in uploaded file."
                    End If
                Else
                    flagCheckDept = 2
                ...
                If flagCheckDept = 1 Or flagCheckDept = 2 Then
                    If (dr("DepartmentId") <> "") Then
                        ...
                        If (dr("DepartmentId") = "") Then
                            strLog = strLog & Environment.NewLine & strErrorDept & Environment.NewLine & currentDateTime & "--" & "Vehicle department name (" & dr("DepartmentId") & ") is not found. Check Row  " & rowIndex & " & column 5 in uploaded file."
                            isDirty = True
                        Else
                            flagCheckDept = 0
                        End If
    ...
    Private Function InsertRecord(dr As DataRow, dtSelectedFuels As DataTable) As Integer ', listOfSites As List(Of Integer)
        Try
            ...
            Dim result As Integer = OBJMaster.SaveUpdateVehicle(0, dr("VehicleName"), dr("Make"), dr("Model"), IIf(dr("Year") = "", "-1", dr("Year")), dr("LicensePlateNumber"), dr("VIN"), dr("Type"), dr("Extension"), "", "-1", dr("DepartmentId"),
                                                            ...
                                                            "-1", dr("Hours"), 1, dr("LicenseState"), 1, "", True, "", 0, 0, "", False)
