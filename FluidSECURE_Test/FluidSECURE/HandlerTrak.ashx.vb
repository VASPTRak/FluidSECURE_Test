...
Public Class HandlerTrak
		...
		Try
			...
			If Route = "Register" Then
				...
				If (RequestFromAPP = "AP") Then
					...
					Dim result As Integer = OBJMasterBAL.UpdateIMEI_UDIDFromHubName(Name, IMEI_UDID, Mobile)
					If (result = -1) Then
			...
			ElseIf Route = "GetVehicleByFSTagMacAddress" Then
				...
				Else
					...
					ErrorInAuthontication(context, "fail", "Not a valid Email and IMEI. Please contact  administrator.")
				End If
			ElseIf Route = "UpgradeCurrentTLDVersionWithUgradableTLDVersion" Then
				Dim checkValid As Boolean = CheckValidRequest(Email, Imei)
				If (checkValid = True) Then
					context.Request.InputStream.Position = 0
					UpgradeCurrentTLDVersionWithUgradableTLDVersion(context)
				Else
					log.Error("Not a valid Email and IMEI.  Email: " & Email & " , IMEI : " & Imei)
					ErrorInAuthontication(context, "fail", "Not a valid Email and IMEI. Please contact  administrator.")
				End If
			ElseIf Route = "SureMDMRebootDevice" Then
				Dim checkValid As Boolean = CheckValidRequest(Email, Imei)
				If (checkValid = True) Then
					context.Request.InputStream.Position = 0
					sendSureMDMRequest(context, Imei, "SureMDMRebootDevice")
				Else
					log.Error("Not a valid Email and IMEI.  Email: " & Email & " , IMEI : " & Imei)
					ErrorInAuthontication(context, "fail", "Not a valid Email and IMEI. Please contact  administrator.")
				End If
	...
	Public Function Authenticate(imei As String, email As String, ByRef objUserData As UserData) As Boolean
		...
		Catch ex As Exception
			log.Error("Exception occurred in Authenticate. Exception is :" & ex.Message & " . in step : " & steps)
		End Try
	...
	Public Function funRegisterUser(context As HttpContext, Name As String, Mobile As String, Email As String, IMEI_UDID As String, ReqFrom As String, companyId As Integer, IsAP As Boolean) As String
	   ...
	   .IsPersonnelPINRequire = False,
		...
		Catch ex As Exception
			log.Error("Exception occurred in funRegisterUser. Exception is :" & ex.Message & " . in step : " & steps)
		End Try
	...
	Public Sub SSIDdsToJson(context As HttpContext, ds As DataSet, Data As UserData, personId As Integer, IMEI_UDID As String)
			...
			If Not ds Is Nothing Then
				If ds.Tables.Count > 0 And ds.Tables(0).Rows.Count > 0 Then
					...
					For index As Integer = 0 To ds.Tables(0).Rows.Count - 1
						...
						objSSIDData.PROBEMacAddress = row("PROBEMacAddress").ToString()
						'Add Launched TLD Firware 
						Dim dtTLDFirmwares As DataTable = New DataTable()
						dtTLDFirmwares = OBJMasterBAL.GetLaunchedTLDFirmwareDetails()
						Dim TLDFirmwareVersion As String = ""
						Dim TLDFilePath As String = ""

						If (Not dtTLDFirmwares Is Nothing) Then
							If (dtTLDFirmwares.Rows.Count > 0) Then
								TLDFirmwareVersion = dtTLDFirmwares.Rows(0)("Version")
								TLDFilePath = "" + HttpContext.Current.Request.Url.Scheme + "://" + HttpContext.Current.Request.Url.Authority & dtTLDFirmwares.Rows(0)("TLDFirmwareFilePath")
							End If
						End If
						objSSIDData.TLDFirmwareFilePath = TLDFilePath
						objSSIDData.TLDFIrmwareVersion = TLDFirmwareVersion

						'check is upgradable or not
						Dim resultTLDUpgradable As String = ""
						Dim TankId As String = ""
						OBJMasterBAL = New MasterBAL()
						Dim dsTLDUpgrade As DataSet
						dsTLDUpgrade = OBJMasterBAL.checkLaunchedAndExistedVersionAndUpdateTLDFirmware(row("HoseId"), "", personId, 0)

						If dsTLDUpgrade IsNot Nothing Then
							If dsTLDUpgrade.Tables(0) IsNot Nothing Then
								If dsTLDUpgrade.Tables(0).Rows.Count > 0 Then
									resultTLDUpgradable = dsTLDUpgrade.Tables(0).Rows(0)("resultUpgradable")
									TankId = IIf(dsTLDUpgrade.Tables(0).Rows(0)("TankId") IsNot Nothing, dsTLDUpgrade.Tables(0).Rows(0)("TankId"), "0")
								Else
									resultTLDUpgradable = "N"
								End If
							Else
								resultTLDUpgradable = "N"
							End If
						Else
							resultTLDUpgradable = "N"
						End If
						objSSIDData.IsTLDFirmwareUpgrade = resultTLDUpgradable
						objSSIDData.TankId = TankId
		...
		Catch ex As Exception
			log.Error("Exception occurred in SSIDdsToJson. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("SSIDdsToJson Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInAuthontication(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
...
#Region "AuthorizationSequence"
	Private Sub AuthorizationSequence(context As HttpContext)
		...
		Catch ex As Exception
			...
			log.Error("Exception occurred in AuthorizationSequence. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("AuthorizationSequence Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInAuthontication(context, "fail", resourceManager.GetString("HandlerMsg15"))
	...
	Private Sub CheckFuelType(context As HttpContext, data As String, VehicleId As Integer, dtVehicle As DataTable, PersonId As Integer,
										...
										If (Decimal.Parse(dsTransactionFuelLimitForDay.Tables(1).Rows(0)("FuelQuantityOfVehicle").ToString()) < vehicleFuelLimitForDay Or vehicleFuelLimitForDay = 0) Or IsGateHub = True Then
											...
											If (IsGateHub = True) Then
												...
												Try
													...
													Dim TimeID As Boolean = False

													Dim dsCurrentTimeInPerson = New DataSet()
													dsCurrentTimeInPerson = OBJMasterBAL.CheckCurrentTimeInTimesTableForGate(SiteId, PersonId)
													Dim flag As Boolean = False
													For Each dr As DataRow In dsCurrentTimeInPerson.Tables(0).Rows
														Try
														...
														If (dr("TimeId") = "1" And dr("StayOpenGate").ToString() = "True") Then
															...
															PumpOffTime = PumpOffTime + diffInSeconds

															Exit For
														ElseIf (dr("StayOpenGate").ToString() = "True") Then
															ToTime = dr("ToTime").ToString()
															FromTime = dr("FromTime").ToString()
															Dim currentDate As Date = DateTime.UtcNow.AddMinutes(OffsetValue).Date.ToString()
															Dim ToDate = currentDate & " " & ToTime.Replace(".", ":")
															Dim FromDate = currentDate & " " & FromTime.Replace(".", ":")
															Dim dateTocheck = Convert.ToDateTime(DateTime.UtcNow.AddMinutes(OffsetValue).ToLongDateString & " " & DateTime.UtcNow.AddMinutes(OffsetValue).ToLongTimeString)

															If Convert.ToDateTime(FromDate) < dateTocheck And Convert.ToDateTime(ToDate) < dateTocheck Then
																Continue For
															ElseIf Convert.ToDateTime(FromDate) < dateTocheck Or flag = True Then
																Dim diffInSeconds As Integer = 0
																If flag Then
																	diffInSeconds = (Convert.ToDateTime(ToDate) - Convert.ToDateTime(FromDate)).TotalSeconds + 60
																Else
																	diffInSeconds = (Convert.ToDateTime(ToDate) - DateTime.UtcNow.AddMinutes(OffsetValue)).TotalSeconds + 60
																End If

																Dim check = (Convert.ToDateTime(ToDate) - DateTime.UtcNow.AddMinutes(OffsetValue)).TotalSeconds
																PumpOffTime = PumpOffTime + diffInSeconds
																flag = True
															End If
														Else
															If (dr("TimeId") = "9" Or flag = True) Then
																Exit For
															End If
														End If

														log.Debug("PumpOffTime: " & PumpOffTime)
													Next
											...
											log.Debug("rootOject.ResponceData.PumpOffTime: " & rootOject.ResponceData.PumpOffTime)

											rootOject.ResponceData.IsTLDCall = IIf(dtHose.Rows(0)("PROBEMacAddress").ToString() = "", "False", "True")
											rootOject.ResponceData.PROBEMacAddress = dtHose.Rows(0)("PROBEMacAddress").ToString()
											...
											Dim dtHubInfo As DataTable = New DataTable()
											dtHubInfo = OBJMasterBAL.GetHubExtraInformationByIMEI_UDID(IMEI_UDID)
											If dtHubInfo IsNot Nothing Then
												If dtHubInfo.Rows.Count > 0 Then
													rootOject.ResponceData.EnablePrinter = dtHubInfo.Rows(0)("EnablePrinter")
													log.Debug("GetHubExtraInformationByIMEI_UDID - 1 - " & IMEI_UDID)
												Else
													rootOject.ResponceData.EnablePrinter = "False"
													log.Debug("GetHubExtraInformationByIMEI_UDID - 2 - " & IMEI_UDID)
												End If
											Else
												rootOject.ResponceData.EnablePrinter = "False"
												log.Debug("GetHubExtraInformationByIMEI_UDID - 3 - " & IMEI_UDID)
											End If
		...
		Catch ex As Exception
			log.Error("Exception occurred in CheckFuelType. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("CheckFuelType Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInAuthontication(context, "fail", resourceManager.GetString("HandlerMsg15"))
...
#End Region
	...
	Private Sub GetSSIDForIphone(data2 As String, Imei As String, context As HttpContext, userObj As UserData)
		...
		Catch ex As Exception
			log.Error("Exception occurred in GetSSIDForIphone. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("GetSSIDForIphone Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInAuthontication(context, "fail", resourceManager.GetString("HandlerMsg15"))
	...
	Private Sub CheckVehicleRequireOdometerEntryAndRequireHourEntry(context As HttpContext)
		...
		Catch ex As Exception
			log.Error("Exception occurred in CheckVehicleRequireOdometerEntryAndRequireHourEntry. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("CheckVehicleRequireOdometerEntryAndRequireHourEntry Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInRequireOdoVehicle(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
	End Sub
	...
	Private Sub CreateResponseForRequireOdoVehicle(context As HttpContext, data As String, VehicleId As Integer, dtVehicle As DataTable, PersonId As Integer, dt As DataTable, RoleId As String, IsGateHub As Boolean) ', IsOdometerRequire As Boolean
		...
		Catch ex As Exception
			log.Error("Exception occurred in CreateResponseForRequireOdoVehicle. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("CreateResponseForRequireOdoVehicle Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInRequireOdoVehicle(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
...
#Region "Pre-AuthTransactions"
	...
	Public Function PreAuthTransactionsDetails(personId As Integer) As ResponsePreAuthTransactions
		...
		Catch ex As Exception
			log.Error("Exception occurred in PreAuthTransactionsDetails. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("PreAuthTransactionsDetails Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
		End Try
...
#End Region
	...
	Private Sub CheckVehicleFobOnly(context As HttpContext, IMEI As String)
		...
		Catch ex As Exception
			log.Error("Exception occurred in CheckVehicleFobOnly. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("CheckVehicleFobOnly Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInRequireOdoVehicle(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
	End Sub
	...
	Private Sub SaveTankMonitorReading(context As HttpContext, IMEI As String)
		Try
			...
			Using inputStream = New StreamReader(context.Request.InputStream)
				...
				ElseIf (String.IsNullOrEmpty(TLDTemperature)) Then
					calculatedTempareture = "0"
				Else
		...
		Catch ex As Exception
			log.Error("Exception occurred in SaveTankMonitorReading. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("SaveTankMonitorReading Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInRequireOdoVehicle(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
	End Sub
	...
	Private Sub SaveInventoryVeederTankMonitorReading(context As HttpContext, IMEI As String)
		...
		Catch ex As Exception
			log.Error("Exception occurred in SaveInventoryVeederTankMonitorReading. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("SaveInventoryVeederTankMonitorReading Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInRequireOdoVehicle(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
	...
	Private Sub SaveDeliveryVeederTankMonitorReading(context As HttpContext, IMEI As String)
		...
		Catch ex As Exception
			log.Error("Exception occurred in SaveDeliveryVeederTankMonitorReading. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("SaveDeliveryVeederTankMonitorReading Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInRequireOdoVehicle(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
	End Sub
				...
				If (VehicleRecurringMSG <> "") Then
					Try
						...
						If dsVehicleValuesData IsNot Nothing And dsVehicleValuesData.Rows.Count > 0 Then
							...
							'check and remove colon and lower case
							FSTagMacAddress = FSTagMacAddress.Replace(":", "").Replace(" ", "").ToLower()
	...
	Private Sub CheckAndValidateFSNPDetails(context As HttpContext, IMEI As String)
																										...
																										If dsTransactionFuelLimitForDay.Tables(0).Rows.Count <> 0 And dsTransactionFuelLimitForDay.Tables(1).Rows.Count <> 0 Then
																											If Decimal.Parse(dsTransactionFuelLimitForDay.Tables(0).Rows(0)("FuelQuantityOfPerson").ToString()) < personFuelLimitForDay Or personFuelLimitForDay = 0 Then
																												If Decimal.Parse(dsTransactionFuelLimitForDay.Tables(1).Rows(0)("FuelQuantityOfVehicle").ToString()) < vehicleFuelLimitForDay Or vehicleFuelLimitForDay = 0 Then
																													...
																													If dtHubInfo IsNot Nothing Then
																														If dtHubInfo.Rows.Count > 0 Then
																															rootOject.EnablePrinter = dtHubInfo.Rows(0)("EnablePrinter")
																														Else
																															rootOject.EnablePrinter = "False"
																														End If
																													Else
																														rootOject.EnablePrinter = "False"
																													End If
		...
		Catch ex As Exception
			log.Error("Exception occurred in CheckAndValidateFSNPDetails. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("CheckAndValidateFSNPDetails Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInAuthontication(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
...
#End Region
	...
	Private Sub CheckPersonFobOnly(context As HttpContext, IMEI As String)
		...
		Catch ex As Exception
			log.Error("Exception occurred in CheckPersonFobOnly. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("CheckPersonFobOnly Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInRequireOdoVehicle(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
	...
	Private Sub GetVehicleByFSTagMacAddress(context As HttpContext, IMEI As String)
		...
		Catch ex As Exception
			log.Error("Exception occurred in GetVehicleByFSTagMacAddress. Exception is :" & ex.Message & " . in step : " & steps)
			If (Not ex.InnerException Is Nothing) Then
				log.Error("GetVehicleByFSTagMacAddress Inner Expcetion is :" & ex.InnerException.Message & " .")
			End If
			ErrorInRequireOdoVehicle(context, "fail", resourceManager.GetString("HandlerMsg15"))
		End Try
	End Sub

	Private Function UpgradeCurrentTLDVersionWithUgradableTLDVersion(context As HttpContext)
		Dim requestJson As String = ""
		Try
			...
			Using inputStream = New StreamReader(context.Request.InputStream)
				...
				log.Info("UpgradeCurrentTLDVersionWithUgradableTLDVersion : " & data)

				Dim javaScriptSerializer = New JavaScriptSerializer()
				Dim serJsonDetails = javaScriptSerializer.Deserialize(data, GetType(UpgradeCurrentTLDVersionWithUgradableTLDVersionMaster))
				Dim TankId = DirectCast(serJsonDetails, UpgradeCurrentTLDVersionWithUgradableTLDVersionMaster).TankId

				Dim Version = DirectCast(serJsonDetails, UpgradeCurrentTLDVersionWithUgradableTLDVersionMaster).Version

				Dim UpgradeCurrentVersionWithUgradableVersionResponseObj = New UpgradeCurrentTLDVersionWithUgradableTLDVersionResponse()
				Dim json As String

				log.Debug("Update TankId : " & TankId)
				log.Debug("With Version : " & Version)
				...
				If (TankId <> "") Then
					If (Version <> "") Then
						Dim dsUpgrade As DataSet
						dsUpgrade = OBJMasterBAL.checkLaunchedAndExistedVersionAndUpdateTLDFirmware(TankId, Version, 0, 1)
						Dim resultUpgradable As String = ""
						If dsUpgrade IsNot Nothing Then
							If dsUpgrade.Tables(0) IsNot Nothing Then
								If dsUpgrade.Tables(0).Rows.Count > 0 Then
				...
				Else
					UpgradeCurrentVersionWithUgradableVersionResponseObj.ResponceMessage = "fail"
					UpgradeCurrentVersionWithUgradableVersionResponseObj.ResponceText = "Error occurred during Update:TankId blank !"
				End If
		...
		Catch ex As Exception
			...
			log.Error("Exception occurred while prcessing request in UpgradeCurrentTLDVersionWithUgradableTLDVersion. Exception is :" & ex.Message & " . Details : " & requestJson)
	...
	Private Function sendSureMDMRequest(context As HttpContext, IMEI_UDID As String, RequestType As String)
		Dim rootOject = New SureMDMResponse()
		Dim json As String = ""
		Dim javaScriptSerializer = New JavaScriptSerializer()
		Try
			...
			If dsIMEI.Tables(0) IsNot Nothing Then
				If dsIMEI.Tables(0).Rows.Count > 0 Then
					CustomerName = dsIMEI.Tables(0).Rows(0)("CustomerName").ToString()
				End If
			End If

			Dim _url As String = "https://trakengineeringinc.suremdm.io/api/group"
			Dim request As HttpWebRequest = HttpWebRequest.Create(_url)

			request.Method = "GET"
			request.Headers.Add("Authorization", "Basic c3dpanRtYW5zQHRyYWtlbmcuY29tOlRyYWtAMTIz")
			request.Headers.Add("ApiKey", "0010BFE8-ACE2-46A0-B939-00D2C4730017")
			...
			If response.StatusCode = HttpStatusCode.OK Then
				...
				Dim serJsonDetails = javaScriptSerializer.Deserialize(responseData, GetType(SureMDMDataDetails))
				If serJsonDetails IsNot Nothing Then
					Dim groups = DirectCast(serJsonDetails, Fuel_Secure.SureMDMDataDetails).Groups
					For Each record As Groups In groups
						If record.GroupName = CustomerName Then
							GroupId = record.GroupID
							Exit For
						End If
					Next
					If GroupId IsNot "" Then
						_url = "https://trakengineeringinc.suremdm.io/api/device?GroupId=" & GroupId
						...
						If response.StatusCode = HttpStatusCode.OK Then
							...
							Dim twitterObject = JToken.Parse(responseData)

							Dim trendsArray = twitterObject.Children.ToArray
							Dim DeviceID As String = ""
							Dim DeviceName As String = ""
							For i = 0 To trendsArray.Count - 1
								If trendsArray(i)("IMEI") = IMEI_UDID Then
									DeviceID = trendsArray(i)("DeviceID").ToString()
									DeviceName = trendsArray(i)("DeviceName").ToString()
									Exit For
								End If
							Next
							If RequestType = "SureMDMRebootDevice" Then
								SureMDMRebootDevice(context, IMEI_UDID, DeviceID, CustomerName, DeviceName)
							End If
							...
							context.Response.Write(json)
						Else
							log.Error("Exception occurred while processing request in sendSureMDMRequest.")
							rootOject.ResponceMessage = "fail"
							rootOject.ResponceText = "Error while processing SURE MDM API"
							json = javaScriptSerializer.Serialize(rootOject)
							context.Response.Write(json)
							Return ""
						End If
	...
	Private Function SureMDMRebootDevice(context As HttpContext, IMEI_UDID As String, DeviceID As String, CustomerName As String, DeviceName As String)
		Dim jsonError As String = ""
		Dim javaScriptSerializer = New JavaScriptSerializer()
		Try
			'----- Send Email

			SendRebbotEmail(CustomerName, DeviceName)

			Dim _url As String = "https://trakengineeringinc.suremdm.io/api/dynamicjob"
			...
			Dim json = "{""DeviceId"":""" & DeviceID & """ , ""JobType"":""Reboot""}"
		...
		Catch ex As Exception
			log.Error("Exception occurred while processing request in SureMDMRebootDevice. Exception is :" & ex.Message)
			Dim rootOject = New SureMDMResponse()
			rootOject.ResponceMessage = "fail"
			rootOject.ResponceText = "Error while processing SURE MDM API - Reboot"
			jsonError = javaScriptSerializer.Serialize(rootOject)
			context.Response.Write(jsonError)
		End Try
		Return ""
	End Function

	Private Function SendRebbotEmail(CompanyName As String, DeviceName As String)
		Dim body As String = String.Empty

		Using sr As New StreamReader(System.Web.Hosting.HostingEnvironment.MapPath("~/RebootEmailText.txt"))
			body = sr.ReadToEnd()
		End Using

		'------------------
		body = body.Replace("CustomerEmail", ConfigurationManager.AppSettings("RebbotEmailTrakEmail").Trim())
		body = body.Replace("CompanyName", CompanyName)
		...
		Dim messageSend As New MailMessage()
		messageSend.From = New MailAddress(ConfigurationManager.AppSettings("FromEmail"))
		messageSend.[To].Add(New MailAddress(ConfigurationManager.AppSettings("RebbotEmailTrakEmail").Trim()))

		messageSend.Subject = "Device trigger the Reboot service from Sure MDM API due 30 % Battery."
		...
		Try
			mailClient.Send(messageSend)
			log.Debug("Mail sent to: " & ConfigurationManager.AppSettings("RebbotEmailTrakEmail").Trim())
			Return ""
		Catch ex As Exception
			log.Debug("Exception occurred in sending Link Defective emails to EmailId : " & ConfigurationManager.AppSettings("RebbotEmailTrakEmail").Trim() & ". ex is :" & ex.Message)
			'ErrorInAuthontication(context, "fail", "Defective Bluetooth Info Email Send failed.")
			Return ""
		End Try
	End Function
