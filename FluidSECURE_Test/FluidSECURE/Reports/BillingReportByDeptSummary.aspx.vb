...
Public Class BillingReportByDeptSummary
    Inherits System.Web.UI.Page
    Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(BillingReportByDeptSummary))
    ...
    Protected Sub btnGenarateReport_Click(sender As Object, e As EventArgs)
        ...
        Try
			...
			dSTran = OBJMaster.GetBillingRptDetails(startDate.ToString(), endDate.ToString(), strConditions, "DeptSummary", chk_FATransaction.Checked)
			If (Not dSTran Is Nothing) Then

                If (dSTran.Tables(0).Rows.Count <= 0) Then
                    If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
                        Dim writtenData = CreateData()
                        CSCommonHelper.WriteLog("Report Genereated", "Billing Report - Department Summary", "", writtenData, HttpContext.Current.Session("PersonName").ToString() & "(" & HttpContext.Current.Session("PersonEmail").ToString() & ")", HttpContext.Current.Session("IPAddress").ToString(), "fail", "Data not found against selected criteria.")
                    End If
			...
			If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
				Dim writtenData = CreateData()
				CSCommonHelper.WriteLog("Report Genereated", "Billing Report - Department Summary", "", writtenData, HttpContext.Current.Session("PersonName").ToString() & "(" & HttpContext.Current.Session("PersonEmail").ToString() & ")", HttpContext.Current.Session("IPAddress").ToString(), "success", "")
			End If
			...
			Session("BillingReportByDeptSummary") = dSTran
