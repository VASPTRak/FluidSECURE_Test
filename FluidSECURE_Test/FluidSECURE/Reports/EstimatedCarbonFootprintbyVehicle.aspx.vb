...
Public Class EstimatedCarbonFootprintbyVehicle
	Inherits System.Web.UI.Page

	Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(EstimatedCarbonFootprintbyVehicle))
	...
	Protected Sub btnGenarateReport_Click(sender As Object, e As EventArgs)
		...
		Try
			...
			dSTran = OBJMaster.GetTransactionRptDetails(startDate.ToString(), endDate.ToString(), strConditions, "EstimatedCarbon", " order by " & DDL_Order.SelectedValue & " " & ddl_ascDesc.SelectedValue)
			...
			If (Not dSTran Is Nothing) Then
				...
				If (dSTran.Tables.Count < 1 Or dSTran.Tables(0).Rows.Count <= 0) Then
					If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
						...
						CSCommonHelper.WriteLog("Report Genereated", "Estimated Carbon Footprint by Vehicle", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "fail", "Data not found against selected criteria.")
					End If
			...
			If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
				...
				CSCommonHelper.WriteLog("Report Genereated", "Estimated Carbon Footprint by Vehicle", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "success", "")
			End If
			Session("EstimatedCarbonFootprintbyVehicle") = dSTran
		...
		Catch ex As Exception
			If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
				...
				CSCommonHelper.WriteLog("Report Genereated", "Estimated Carbon Footprint by Vehicle", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "fail", "Data not found against selected criteria. Exception is " & ex.Message)
			End If
			ErrorMessage.InnerText = "Data not found against selected criteria."
			ErrorMessage.Visible = True
			ScriptManager.RegisterStartupScript(Me, Me.GetType(), "MSG", "LoadDateTimeControl();loadMultiList();", True)
	...
	Private Function CreateData() As String
		Try
			...
			Dim data As String = "Transaction Date From = " & txtTransactionDateFrom.Text.Replace(",", "") & " ; " &
									...
									"Order by = " & DDL_Order.SelectedItem.Text.Replace(",", " ") & " ; " &
									"ASC/DESC = " & ddl_ascDesc.SelectedItem.Text.Replace(",", " ") & " ; " &
