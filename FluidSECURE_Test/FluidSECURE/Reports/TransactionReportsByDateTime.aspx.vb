...
Public Class TransactionReportsByDateTime
    Inherits System.Web.UI.Page


    Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(TransactionReportsByDateTime))
    ...
    Protected Sub btnGenarateReport_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            'If (HDF_VehicleId.Value <> "0") Then
            '    strConditions = IIf(strConditions = "", " and T.VehicleID = " + HDF_VehicleId.Value, strConditions + " and T.VehicleID = " + HDF_VehicleId.Value)
            'End If
        ...
        Catch ex As Exception
            ...
            If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
                ...
                CSCommonHelper.WriteLog("Report Genereated", "Transaction Report By Date Time", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "fail", "Data not found against selected criteria. Exception is : " & ex.Message)
            End If
