...
Public Class TransactionReportByVehicle
    Inherits System.Web.UI.Page

    Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(TransactionReportByVehicle))
    ...
    Protected Sub btnGenarateReport_Click(sender As Object, e As EventArgs)
        ...
        Try
			...
			dSTran = OBJMaster.GetTransactionRptDetails(startDate.ToString(), endDate.ToString(), strConditions, "vehicle", chk_FATransaction.Checked)
			If (Not dSTran Is Nothing) Then
                ...
                If (dSTran.Tables.Count < 3 Or dSTran.Tables(0).Rows.Count <= 0) Then
                    If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
                        ...
                        CSCommonHelper.WriteLog("Report Genereated", "Transaction Report by Vehicle", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "fail", "Data not found against selected criteria.")
                    End If
			...
			If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
				...
				CSCommonHelper.WriteLog("Report Genereated", "Transaction Report by Vehicle", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "success", "")
			End If
			...
			Session("TransReportByVehicle") = dSTran
            ...
            Response.Redirect("~/Reports/TransactionReportByVehicleReport")
        ...
        Catch ex As Exception
            If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
                ...
                CSCommonHelper.WriteLog("Report Genereated", "Transaction Report by Vehicle", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "fail", "Data not found against selected criteria. Exception is " & ex.Message)
            End If
            ErrorMessage.InnerText = "Data not found against selected criteria."
            ErrorMessage.Visible = True
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "MSG", "LoadDateTimeControl();loadMultiList();", True)
