...
Public Class PersonnelRPT
	Inherits System.Web.UI.Page
	Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(PersonnelRPT))
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		...
		Try
			...
			Else
                ...
                rep.Refresh()
                Dim PersonnelReportPath As String = ConfigurationManager.AppSettings("PersonnelReportPath")
                rep.ReportPath = Server.MapPath(PersonnelReportPath)

                Dim UserRoleAccess As String = 1

                If (Session("RoleName").ToString() <> "SuperAdmin" And Session("RoleName").ToString() <> "CustomerAdmin") Then
                    UserRoleAccess = 0
                End If

                Dim UserRole As ReportParameter = New ReportParameter("UserRole", UserRoleAccess)

                    RPT_Personnel.LocalReport.SetParameters(New ReportParameter() {UserRole})
		...
		Catch ex As Exception
			...
			Response.Redirect("~/Reports/PersonnelReport")
		End Try
	End Sub
