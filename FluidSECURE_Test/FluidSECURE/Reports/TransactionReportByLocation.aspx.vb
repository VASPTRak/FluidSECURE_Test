...
Public Class TransactionReportByLocation
	...
	Private Sub BindSites(CustomerId As Integer, location As String)
		Try
			...
			If (location = "Select ALL Locations") Then
			...
			Else
				dtSites = OBJMaster.GetSiteByCondition(" And SiteAddress = '" + location + "' And c.CustomerId =" + CustomerId.ToString(), Session("PersonId").ToString(), Session("RoleId").ToString(), IsDeletedLinkAllow)
			End If
	...
	Protected Sub DDL_Customer_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDL_Customer.SelectedIndexChanged
		Try
			...
			BindLocations(Convert.ToInt32(DDL_Customer.SelectedValue))
			BindAllPersonnels(0)
			BindAllHubs()
			BindAllVehicles(0)
	...
	Protected Sub btnGenarateReport_Click(sender As Object, e As EventArgs)
		...
		Try
			...
			If (DDl_Locations.SelectedValue <> "0") Then
				strConditions = IIf(strConditions = "", " and s.SiteAddress = '" + DDl_Locations.SelectedValue + "' ", strConditions + " and s.SiteAddress = '" + DDl_Locations.SelectedValue + "' ")
			End If
			...
			dSTran = OBJMaster.GetTransactionRptDetails(startDate.ToString(), endDate.ToString(), strConditions, "site", chk_FATransaction.Checked)
			If (Not dSTran Is Nothing) Then
				...
				If (dSTran.Tables.Count < 3 Or dSTran.Tables(0).Rows.Count <= 0) Then
					If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
						...
						CSCommonHelper.WriteLog("Report Genereated", "Transaction Report by Location", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "fail", "Data not found against selected criteria.")
					End If
			...
			If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
				...
				CSCommonHelper.WriteLog("Report Genereated", "Transaction Report by Location", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "sucess", "")
			End If
			...
			Session("TransReportByLocation") = dSTran
            ...
            Response.Redirect("~/Reports/TransactionReportByLocationReport")
		...
		Catch ex As Exception
			If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
				...
				CSCommonHelper.WriteLog("Report Genereated", "ransaction Report by Location", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "fail", "Data not found against selected criteria. Exception is " & ex.Message)
			End If
			ErrorMessage.InnerText = "Data not found against selected criteria."
			ErrorMessage.Visible = True
			ScriptManager.RegisterStartupScript(Me, Me.GetType(), "MSG", "LoadDateTimeControl();loadMultiList();", True)
	...
	Private Sub BindLocations(CustomerId As Integer)
		Try
			...
			If chk_IsDeletedLinkAllow.Checked Then
				IsDeletedLinkAllow = True
			End If
			dtLocation = OBJMaster.GetSiteByCondition(" And s.SiteAddress is not null AND s.SiteAddress <> ''  And c.CustomerId =" + CustomerId.ToString(), Session("PersonId").ToString(), Session("RoleId").ToString(), IsDeletedLinkAllow)
			Dim dtview As DataView = dtLocation.DefaultView
			Dim dt As DataTable = dtview.ToTable(True, "SiteAddress")
			DDl_Locations.DataSource = dt
			DDl_Locations.DataValueField = "SiteAddress"
			DDl_Locations.DataTextField = "SiteAddress"
			DDl_Locations.DataBind()
			DDl_Locations.Items.Insert(0, New ListItem("Select ALL Locations", "0"))
			'If DDL_Customer.SelectedValue <> 0 And Not (IsNothing(DDl_Locations.SelectedItem)) Then
			BindSites(Convert.ToInt32(DDL_Customer.SelectedValue), DDl_Locations.SelectedItem.ToString())
			'End If

		Catch ex As Exception

			ErrorMessage.Visible = True
			ErrorMessage.InnerText = "Error occurred  while getting Locations, please try again later."

			log.Error("Error occurred in BindLocations Exception is :" + ex.Message)

		End Try
	End Sub

	Protected Sub DDl_Locations_SelectedIndexChanged(sender As Object, e As EventArgs)
		Try

			BindSites(Convert.ToInt32(DDL_Customer.SelectedValue), DDl_Locations.SelectedItem.ToString().TrimEnd())
		Catch ex As Exception
			log.Error("Error occurred in DDl_Locations_SelectedIndexChanged Exception is :" + ex.Message)
			ErrorMessage.Visible = True
			ErrorMessage.InnerText = "Error occurred while getting data, please try again later."
		Finally
			ScriptManager.RegisterStartupScript(Me, Me.GetType(), "MSG", "LoadDateTimeControl();loadMultiList();$('[id*=lstSites]').multiselect('selectAll', false).multiselect('updateButtonText');", True)
	...
	Protected Sub chk_IsDeletedLinkAllow_CheckedChanged(sender As Object, e As EventArgs)
		Try
			...
			BindLocations(Convert.ToInt32(DDL_Customer.SelectedValue.ToString()))
