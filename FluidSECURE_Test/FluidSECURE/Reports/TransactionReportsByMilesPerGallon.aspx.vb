...
Public Class TransactionReportsByMilesPerGallon
	Inherits System.Web.UI.Page

	Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(TransactionReportsByMilesPerGallon))
	...
	Protected Sub btnGenarateReport_Click(sender As Object, e As EventArgs)
		...
		Try
			...
			If (TXT_MPGKLessThan.Text <> "") Then
				ExtraCondition = " where MPGPerK < " + TXT_MPGKLessThan.Text
			...
			dSTran = OBJMaster.GetTransactionRptDetails(startDate.ToString(), endDate.ToString(), strConditions, "MPGK", chk_FATransaction.Checked, ExtraCondition)
			...
			If (Not dSTran Is Nothing) Then
				...
				If (dSTran.Tables.Count < 3 Or dSTran.Tables(0).Rows.Count <= 0) Then
					If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
						...
						CSCommonHelper.WriteLog("Report Genereated", "Transaction Report By Miles Per Gallon/Kilometer", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "fail", "Data not found against selected criteria.")
					End If
			...
			If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
				...
				CSCommonHelper.WriteLog("Report Genereated", "Transaction Report by Miles Per Gallon/Kilometer", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "success", "")
			End If
			...
			Session("TransReportByMPGPerK") = dSTran
		...
		Catch ex As Exception
			If (ConfigurationManager.AppSettings("AllowActivityLogin").ToString().ToLower() = "yes") Then
				...
				CSCommonHelper.WriteLog("Report Genereated", "Transaction Report by Miles Per Gallon/Kilometer", "", writtenData, Session("PersonName").ToString() & "(" & Session("PersonEmail").ToString() & ")", Session("IPAddress").ToString(), "fail", "Data not found against selected criteria. Exception is " & ex.Message)
			End If
			ErrorMessage.InnerText = "Data not found against selected criteria."
			ErrorMessage.Visible = True
			ScriptManager.RegisterStartupScript(Me, Me.GetType(), "MSG", "LoadDateTimeControl();loadMultiList();", True)
	...
	Private Function CreateData() As String
		Try
			...
			Dim data As String = "Transaction Date From = " & txtTransactionDateFrom.Text.Replace(",", "") & " ; " &
									...
									"MPG/K less than = " & TXT_MPGKLessThan.Text.Replace(",", "") & " ; " &
									"MPG/K greater than = " & TXT_MPGKGreaterThan.Text.Replace(",", "") & " ; "
